...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int]):
        ...
        strides = tuple(
            islice(
                accumulate(new_shape, func=lambda a, b: a // b, initial=totalSize),
                1,
                len(new_shape) + 1,
            )
        )
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert all(
            oldAxis == 1 or newAxis == oldAxis
            for newAxis, oldAxis in zip(new_shape, self.shape)
        )
        newStrides = tuple(
            stride if oldAxis == newAxis else 0
            for stride, oldAxis, newAxis in zip(self.strides, self.shape, new_shape)
        )
        return NDArray.make(
            shape=new_shape,
            strides=newStrides,
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        newShape = tuple(math.ceil((idx.stop - idx.start) / idx.step) for idx in idxs)
